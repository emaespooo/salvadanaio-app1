...
with tab_storico:
    ...
    if not df_clean.empty:
        ...
        st.info("Nessuno storico disponibile.")import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
from datetime import datetime
...
with tab_dashboard:
    ...
    if not df_clean.empty:
        ...
        with kpi_col2:
            st.metric("📉 USCITE TOTALI", f"{totale_spese:,.2f} €")
        with kpi_col3:
            st.metric("💰 SALDO NETTO", f"{saldo_totale:,.2f} €", 
                      delta=f"{'+' if saldo_totale >= 0 else ''}{saldo_totale:,.2f} €", 
                      delta_color="normal" if saldo_totale >= 0 else "inverse")
        ...
        for idx, (_, row) in enumerate(resoconto_mensile.iloc[::-1].iterrows()):
            ...
            with st.expander(f"**{mese_corrente}** &nbsp;|&nbsp; Netto: {icona_stato} **{utile_mese:,.2f} €**", expanded=(idx == 0)):
                col_m_left, col_m_right = st.columns([1, 1])
                with col_m_left:
                    m1, m2, m3 = st.columns(3)
